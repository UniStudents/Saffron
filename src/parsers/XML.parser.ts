...
export class XMLParser extends Parser {
    validateScrape(s?: SourceScrape): void {
        ...
        if(typeof scrape.unpairedTags !== 'undefined' && (!Array.isArray(scrape.unpairedTags) || scrape.unpairedTags.some(a => typeof a !== 'string')))
            throw new Error(`unpairedTags must be a string array`);
    }

    assignInstructions(instructions: Instructions, scrape?: SourceScrape): void {
        instructions.xml = scrape as ScrapeXML;
    }
    ...
    async parse(result: RequestsResult, utils: Utils): Promise<Article[]> {
        ...
        for (const index in root) {
            ...
            if (this.isSkippedJSONNode(instructions.xml.skip!, child, index as any)) continue;

            const articleData: Record<string, any> = {};
            const options = instructions.xml.article;
            ...
            Object.entries(articleData).forEach(extra => {
                if (XMLParser.BASIC_DATA.includes(extra[0]) || !extra[1]) return;
