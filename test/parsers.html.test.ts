...
describe("HTML parser", function () {
    it('Test 1', function () {
        return Saffron.parse(require('./sources/html/html1.json')).then(result => {
            ...
            expect(article.title).to.equal('\n                                            \n                                                Εις μνήμην Χρυσούλας Τόμπρου\n                                            \n                                        ');
            expect(article.content).to.equal('\n' +
                '                                            Στις 26 Αυγούστου έφυγε από κοντά μας,\n' +
    ...
    it('Test 2', function () {
        return Saffron.parse(require('./sources/html/html2.json')).then(result => {
            ...
            const article = obj.articles[0];
            expect(article.title).to.equal('\n                        \\t\\t\\t\\t\\t\\tΕΚΤΑΚΤΗ ΑΝΑΚΟΙΝΩΣΗ ΓΙΑ ΤΗΝ ΟΡΚΩΜΟΣΙΑ ΠΤΥΧΙΟΥΧΩΝ ΙΟΥΝΙΟΥ -ΙΟΥΛΙΟΥ 2022 (ΚΛΙΜ. Β\n                        18-10-2022)\n                        \\t\\t\\t\\t\\t');
    ...
    it('Test 3', function () {
        return Saffron.parse(require('./sources/html/html3.json')).then(result => {
            ...
            const article = obj.articles[0];
            expect(article.title).to.equal('Ηλεκτρονικη\n                                                                                υποβολη διπλωματικων και ερευνητικων\n                                                                                εργασιων στη Βιβλιοθηκη και το Αρχειο\n                                                                                του ΤΑΜΠΠ, Οκτωβρίου - Νοεμβρίου\n                                                                                2022');
    ...
    it('Test 3', function () {
        return Saffron.parse(require('./sources/html/html4.json')).then(result => {
            ...
            const article = obj.articles[0];
            expect(article.title).to.equal('\n                            Αναβολή\n                                Μαθήματος Υδροδυναμική Σχεδίαση Μικρών Σκαφών\n                        ');
            expect(article.content).to.equal('\n\n                        Το μάθημα Υδροδυναμική Σχεδίαση Μικρών Σκαφών της Δευτέρας 7 Νοεμβρίου του κου Γρηγορόπουλου\n                        αναβάλλεται λόγω εκτάκτου γεγονότος.\n\n\n                    ');
            expect(article.link).to.equal('/general_news/1108/anaboli_mathimatos_ydrodynamiki_sxediasi_mikrwn_skafwn');
            expect(article.pubDate).to.equal('\n                            Friday, 04 Νοεμβρίου 2022 - Γενικές Ανακοινώσεις\n                        ');
            expect(article.attachments.length).to.equal(0);
            expect(article.thumbnail).to.be.undefined;
        });
    });
